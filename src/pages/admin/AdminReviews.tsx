...
import { Star, Check, X, Clock, Filter } from "lucide-react";
...
const AdminReviews = () => {
  ...
  const { data: reviews = [], isLoading } = useQuery({
    ...
    queryFn: async () => { const { data, error } = await db("reviews").select("*").order("created_at", { ascending: false }); if (error) throw error; return data || []; },
  });
  ...
  const filtered = useMemo(() => {
    return reviews.filter((r: any) => {
      ...
      if (typeFilter !== "all" && r.reviewable_type !== typeFilter) return false;
      if (ratingFilter !== "all" && r.rating !== Number(ratingFilter)) return false;
      return true;
    });
  }, [reviews, statusFilter, typeFilter, ratingFilter]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Select value={typeFilter} onValueChange={(v) => { setTypeFilter(v); setPage(1); }}>
          ...
          <SelectContent>
            <SelectItem value="all">All Types</SelectItem><SelectItem value="package">Package</SelectItem><SelectItem value="vehicle">Vehicle</SelectItem><SelectItem value="destination">Destination</SelectItem><SelectItem value="hotel">Hotel</SelectItem>
          </SelectContent>
        </Select>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="text-[10px] capitalize">{review.reviewable_type}</Badge>
                      <Badge variant="outline" className={`text-[10px] ${statusColor(review.status)}`}>{review.status}</Badge>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 text-xs text-muted-foreground"><Clock className="w-3 h-3" />{format(new Date(review.created_at), "dd MMM yyyy, hh:mm a")}</div>
                </div>
